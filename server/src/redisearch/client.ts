...
export class RediSearch {
    ...
    async addQuestion(question: IQuestion) {
        ...
        if (indexExists) {
            ...
            const res = await this.search(question.question, { fuzzy: false, onlyTitle: true });
        ...
        this.db.hset(`questions:${uuid()}`,
            'question', question.question,
            'descriptionGood', question.descriptionGood,
            'descriptionBad', question.descriptionBad
        );
    ...
    private async createIndex(recreate = false) {
        ...
        if (exists && !recreate) {
            console.log('[REDISEARCH] Index already exists, use the recreate parameter to force a recreation');
            return;
        }
...
export const defaultQuestions = [
    ...
    {
        descriptionGood: 'Releasing is simple, safe, painless and mostly automated. ',
        descriptionBad:
            'Releasing is risky, painful, lots of  manual work and takes forever.',
        question: 'Easy to release',
    },
    {
        descriptionGood: 'We love going to work and have great fun working together! ',
    ...
    {
        descriptionGood: 'We get stuff  done really quickly! No waiting and no delays. ',
        descriptionBad:
